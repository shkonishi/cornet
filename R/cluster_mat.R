...
cluster_mat <- function(dat, distm, clm, column, method_dycut, y_fctr, x_fctr, rep_fctr){
  ...
  cols.ccl <- vcol[as.factor(dyct)] # sample order

  ## create leaf colour corresponding to dynamiccut cluster ----
  ...
  par(mar=c(0,2,2,0))
  cols.ccl.lo <- cols.ccl[r_hcl$order] # colours label order
  cl_txt <- as.character(unique((dyct[r_hcl$order]))) # number of cluster elements
  cl_num <- table(dyct[r_hcl$order])[cl_txt] # cluster number table
  cl_txt_al <- rep(NA, length(dyct))
  cmsum <- cumsum(cl_num)
  ...
  if (missing(y_fctr) & !missing(rep_fctr)){ # x- and replicate factor, and no y_fctr
    ...
    cl_levels <- as.character(names(cmsum))
    ...
    ## add cluster number and mean of replicate. ----
    cl_levels <- as.character(names(cmsum)) # cluster column's level
